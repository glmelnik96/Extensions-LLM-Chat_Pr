        ...
        function tryEval() {
          cs.evalScript(extendScriptExpr, function (raw) {
            ...
            if (isColdStartGlitch(s)) {
              /* 06.08.2026 (ревью): для мутирующих операций НЕ ретраим.
                 'EvalScript error.' не гарантирует, что host ничего не сделал:
                 операция могла выполниться и упасть на сериализации ответа —
                 повтор = двойное применение резов по уже сдвинутым координатам.
                 Прогрев движка выполняет ensureHost (немутирующий ping). */
              if (mutating) {
                clearTimeout(timer);
                finish(new Error('ExtendScript вернул ошибку на мутирующей операции. НЕ запускайте её повторно вслепую: сначала проверьте таймлайн (операция могла примениться) и при необходимости откатите через Edit → Undo. raw=' + s), null);
                return;
              }
              attempt++;
              if (attempt < DELAYS.length) {
