...
(function (global) {
  ...
  var TIER0_CORE = [
    ...
    '',
    'КОНТРОЛЬ РЕЗУЛЬТАТА МУТАЦИИ:',
    '• После каждого apply_* смотри _timelineDiff: если match===false — сообщи пользователю о расхождении и ОСТАНОВИСЬ, не продолжай резать по старому плану.',
    '• Прежде чем сказать «готово», сверь фактический deltaDurationSec с ожидаемым — при промахе >0.5с вызови get_timeline_snapshot и разберись.',
    '',
  ...
  var TIER1_TRANSCRIPT = [
    ...
    '  и начинается суть. Всё до сути = removeInterval; если совпадает с целыми абзацами — предпочти removeRefs.',
    '',
    ...
    '',
    '«Сожми до N минут» / «сократи сохранив суть» / «собери по смыслу» / «уложи в N секунд» / «сделай короткую версию» →',
    '  ИСПОЛЬЗУЙ propose_montage_plan (НЕ голый propose_transcript_cuts):',
    '  1. get_timeline_snapshot → sequenceName. (get_transcript_structure ЗДЕСЬ НЕ нужен.)',
    '  2. ОДИН propose_montage_plan({ sequenceKey, targetDurationSec, summary }). blocks передавать НЕ нужно —',
    '     плагин сам размечает смыслы транскрипта второй моделью, собирает план keep/cut под цель ±10%',
    '     и показывает пользователю карточку на подтверждение.',
    '',
    ...
    '• propose_transcript_cuts({ removeRefs | removeIntervals | keepIntervals, summary, sequenceKey, paddingSec }) — план вырезок.',
    '  ПРЕДПОЧИТАЙ removeRefs после get_transcript_structure — ссылки на абзацы по индексу (поле i).',
    '  Плагин сам развернёт индексы в точные секунды из кэша. removeIntervals с ручными секундами —',
    '  только когда режешь НЕ по границам структуры (результаты analyze_transcript_for_cuts).',
    '  Передай removeRefs/removeIntervals ИЛИ keepIntervals — не оба сразу.',
    ...
    '  removeIntervals — для точечных вырезок при ручных секундах или из analyze_transcript_for_cuts.',
    ...
    '• Предпочитай removeRefs:[{paragraph: i}] — плагин сам разрешит индексы в секунды. removeIntervals с ручными секундами — fallback для analyze_transcript_for_cuts или вырезки не по границам абзацев.',
    ...
    '  ✗ ОШИБКА: конец keepInterval на сегменте БЕЗ финальной пунктуации (. ! ?) — мысль',
    '    оборвётся на слух. endSec keep-фрагмента ставь на конец сегмента, чей текст',
    '    завершается точкой/«!»/«?»; startSec — на начало предложения. Собирай ЦЕЛЫЕ мысли.',
    ...
    '  ✗ ОШИБКА: при failedCount>0 / _partialFailureHint повторять ВЕСЬ план целиком.',
    '    Пересылай ТОЛЬКО упавшие интервалы/операции (results[].ok===false), исправив причину. Успешные уже применены, координаты сдвинулись.',
    ''
  ].join('\n');
  ...
  function classifyIntent(text) {
    ...
    if (/парази|вступлен|оговор|повтор|вырез|вырежь|почист|убери|монтаж|текст|транскрипт|filler|intro|outro|outtake|собери|уложи|сожм|сократ|хронометраж|тем[аеуы]|про что|о чём|ключев|фрагмент|смысл|динамич|скучн|интересн|коротк|длинн|мусор|вод[аеуы]|пуст|пауз|тишин|молчан|болтовн/.test(t)) {
